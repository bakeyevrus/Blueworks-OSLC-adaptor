...
public class BlueworksConnector extends IRequirementsConnector {
	...
	@Override
	public List<RequirementInfo> getRequirementsFromServiceProvider(
			String providerId) throws UnauthorizedException {
		...
		for (Iterator<BlueworksProcess> iterator = processes.iterator(); iterator
				.hasNext();) {
			BlueworksProcess blueworksProcess = (BlueworksProcess) iterator
					.next();
			...
			for (Iterator<BlueworksProcessActivity> activitiesIterator = blueworksProcess
					.getProcessActivities().iterator(); activitiesIterator
					.hasNext();) {
				BlueworksProcessActivity blueworksProcessActivity = (BlueworksProcessActivity) activitiesIterator
						.next();

				// Convert each blueworks activity into a requirementinfo and
				// put it in the list
				activities.add(blueworksProcessActivity.toRequirementInfo());

			}

			// Now let's create the new RequirementInfo object, add in the
			// process level data and add the activities list as the children
			// RequirementInfo objects
			// TODO - sort out passing the modified date through from Blueworks
			// to the RequirementInfo obect
			requirements.add(new RequirementInfo(blueworksProcess
					.getProcessId(), blueworksProcess.getName(), "process",
					"dummy documentation", blueworksProcess.getModifiedDate(),
					blueworksProcess.getModifiedUserName(), "process", "",
					activities));
	...
	public RequirementInfo getRequirementById(String processId) {
		...
		try {
			// Let's try and get the process associated with the processId. Note
			// that the processId may actually be an activity Id. We'll get the
			// parent process back along with the associated child processes.
	...
	public URI getEndpointURIForRequirement(RequirementInfo req) {
		...
		try {
			addressOfRequirementOnEndpoint = new URI(
					getEndpointStringForRequirement(req));
	...
	public String getEndpointStringForRequirement(RequirementInfo req) {
		...
		if (req.getType() != null
				&& "activity".equals(req.getType().toLowerCase())) {
			uriString = BLUEWORKS_BASE_URL + "scr/processes/" + req.getParent()
					+ "?processMode=VIEW&activityID=" + req.getId() + "#map";
		} else if (req.getType() != null
				&& "process".equals(req.getType().toLowerCase())) {
