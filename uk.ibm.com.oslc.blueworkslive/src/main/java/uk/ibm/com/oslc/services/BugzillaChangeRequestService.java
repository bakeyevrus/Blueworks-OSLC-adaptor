	...
	
{
    ...
    public List<OSLCRequirement> getChangeRequests(@PathParam("productId")         final String productId,
                                      ...
                                      @QueryParam("oslc.searchTerms") final String searchTerms) throws ServletException, IOException
	{
        ...
        if (results != null) {
        	...
        	RequestDispatcher rd = httpServletRequest.getRequestDispatcher("/rm/changerequest_collection_html.jsp");
                                                  ...
                                                  @QueryParam("oslc.prefix")     final String prefix) throws IOException, ServletException, URISyntaxException
    {
        ...
        final RequirementInfo req = IRequirementsConnector.getInstance().getRequirementById(changeRequestId);
        ...
        if (req != null) {
        	...
        	changeRequest.setAbout(getAboutURI(productId + "/requirements/" + changeRequest.getIdentifier()));
           ...
           throws URISyntaxException, IOException, ServletException
    {
        ...
        if (changeRequest != null) {      	
            ...
            compact.setAbout(getAboutURI(productId + "/requirements/" + changeRequest.getIdentifier()));
            compact.setTitle(changeRequest.getTitle());
            
            String iconUri = null;
            if (changeRequest.getRequirementType() != null && "ACTIVITY".equals(changeRequest.getRequirementType()))
            {
            	iconUri = ApplicationManager.getServletBase() + "/images/resources/BlueworksActivity16.gif";
            }
            else
            {
                iconUri = ApplicationManager.getServletBase() + "/images/resources/BlueworksProcess16.gif";
            }
	...
	private OSLCRequirement getRequirementFromId(final String requirementId) {
		final RequirementInfo info = IRequirementsConnector.getInstance().getRequirementById(requirementId);
		OSLCRequirement changeRequest = null;
		if (info != null)
		{
			try {
	...
	{	
		RequirementInfo info = IRequirementsConnector.getInstance().getRequirementById(changeRequestId);
		String forwardUri = IRequirementsConnector.getInstance().getEndpointStringForRequirement(info);
						              ...
						              @PathParam("productId")  final String productId) throws ServletException, IOException
	{
		...
		httpServletRequest.setAttribute("baseURL", ApplicationManager.getServletBase());
		...
		if (terms != null ) {
			...
			try {	
                RequestDispatcher rd = httpServletRequest.getRequestDispatcher("/rm/changerequest_selector.jsp"); 
	    		rd.forward(httpServletRequest, httpServletResponse);
			                 ...
			                 @PathParam("changeRequestId") final String changeRequestId) throws ServletException, IOException, URISyntaxException
	{
        ...
        if (changeRequest != null) {
        	...
        	httpServletRequest.setAttribute("processType", changeRequest.getRequirementType());
        	
        	RequestDispatcher rd = httpServletRequest.getRequestDispatcher("/rm/changerequest_preview_small.jsp");
            rd.forward(httpServletRequest,httpServletResponse);
        	return;
        }
    ...
    public Response addChangeRequest(@PathParam("productId") final String productId,
                                                             final OSLCRequirement changeRequest) throws IOException, ServletException
    ...
    public void changeRequestCreator(@PathParam("productId") final String productId) throws IOException, ServletException
    {
    	try {				
	        ...
	        RequestDispatcher rd = httpServletRequest.getRequestDispatcher("/rm/changerequest_creator.jsp");
    		rd.forward(httpServletRequest, httpServletResponse);
    ...
    {    	
    	throw new RuntimeException("creating new blueworks requirements is unsupported in this version.");
    }
    		                                                          ...
    		                                                          final OSLCRequirement       changeRequest) throws IOException, ServletException
    {
    	...
    	throw new RuntimeException("updating blueworks requirements is unsupported in this version.");
    }
	...
	protected URI getAboutURI(final String fragment)
	{
		URI about;
		try {
			about = new URI(ApplicationManager.getServicePath() + "/" + fragment);
		} catch (URISyntaxException e) {
			throw new WebApplicationException(e);
		}
		return about;
			...
			throws ServletException, IOException 
	{
		...
		try {
			...
			if (terms.isEmpty())
			{	
				List<RequirementInfo> requirements = IRequirementsConnector.getInstance().getRequirementsFromServiceProvider(productId);
			
				for (RequirementInfo requirementInfo : requirements) {
					OSLCRequirement changeRequest = OSLCRequirement.fromRequirement(requirementInfo);
					changeRequest.setAbout(getAboutURI(productId + "/requirements/"+ changeRequest.getIdentifier()));
			...
			// a search string must have been provided, so let's search for it...
			else
			{
				List<RequirementInfo> requirements = IRequirementsConnector.getInstance().getRequirementsFromServiceProvider(productId, terms);
				...
				for (RequirementInfo requirementInfo : requirements) {
					...
					System.out.println("The time3 is: " + (endTime3 - startTime) + " ms");
				}
			
			}

			// just set the service provider for each and every change request - easier to do it here
			for (OSLCRequirement cr : results) {
				cr.setServiceProvider(ServiceProviderCatalogSingleton.getServiceProvider(httpServletRequest, productId).getAbout());  
			}
			...
			//List<OSLCRequirement> results = changeRequestsFromBugList(httpServletRequest, bugList, productId);

	        RequestDispatcher rd = httpServletRequest.getRequestDispatcher("/rm/changerequest_filtered_json.jsp"); 
	        rd.forward(httpServletRequest, httpServletResponse);
	...
	private void setInstanceShape(RequirementInfo requirementInfo, OSLCRequirement changeRequest) {
		...
		try {
			...
			{
				shapeURI = new URI(baseURI+"/images/resources/BlueworksProcess16.gif");
			}
			else if(requirementType !=null && "activity".equals(requirementType))
			{
				shapeURI = new URI(baseURI+"/images/resources/BlueworksActivity16.gif");
			}
			else
			{
				shapeURI = new URI(baseURI+"/images/resources/BlueworksProcess16.gif");
